...
## Performance Allowance Logic
Reinstate the performance reward as a monetary benefit, with the multiplier controlled at the entity level.

### [MODIFY] [server/db/init.js](file:///c:/Users/mathi/Desktop/AntiGravity%20Demos/HRMS%20Singapore/server/db/init.js)
- Add `performance_multiplier` (REAL DEFAULT 0) to `entities` table migration.

### [MODIFY] [server/routes/payroll.js](file:///c:/Users/mathi/Desktop/AntiGravity%20Demos/HRMS%20Singapore/server/routes/payroll.js)
- Fetch the `performance_multiplier` from the `entities` table.
- Stop zeroing out `performanceCredits` before passing to engine.
- Pass the entity multiplier to [processEmployeePayroll](file:///c:/Users/mathi/Desktop/AntiGravity%20Demos/HRMS%20Singapore/server/engine/payroll-engine.js#10-209).
