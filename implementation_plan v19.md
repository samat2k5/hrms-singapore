...
# Making Email Domains Configurable

The user requested the ability to add new email domains to the suggestions dropdown in the future. We will create a configuration interface for Email Domains, similar to Employee Groups or Grades.

## Proposed Changes

### [Component Name] Database Schema
#### [NEW] server/db/migrate-email-domains.js
- Create a script to add a new `email_domains` table (`id`, `entity_id`, `domain`, `created_at`).
#### [MODIFY] server/db/init.js
- Update `CREATE TABLE` to include `email_domains`.

### [Component Name] Backend API
#### [NEW] server/routes/email-domains.js
- Create CRUD operations (GET, POST, PUT, DELETE) restricted to the current user's entity.
#### [MODIFY] server/server.js
- Register the new router.

### [Component Name] Frontend Settings UI
#### [MODIFY] client/src/services/api.js
- Add API call wrappers for email domains.
#### [NEW] client/src/pages/EmailDomains.jsx
- Create a standard CRUD list page, styled like [EmployeeGroups.jsx](file:///c:/Users/mathi/Desktop/AntiGravity%20Demos/HRMS%20Singapore/client/src/pages/EmployeeGroups.jsx).
#### [MODIFY] client/src/App.jsx
- Add the route `/email-domains`.
#### [MODIFY] client/src/components/Layout.jsx
- Add "Email Domains" to the "Master Data" navigation section.

### [Component Name] Employee Form Integration
#### [MODIFY] client/src/pages/EmployeeForm.jsx
- Fetch the `email-domains` list on mount.
- Render the `email-domains` datalist dynamically, mapping the configured domains as options instead of hardcoding them.
