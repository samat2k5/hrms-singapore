...
### Audit & Robustness Fixes (PDF Export)
I've implemented critical hardening for the PDF generation engine to resolve the reported "Cannot read properties of undefined (reading 'data')" error:
- **Robust Imports:** Updated dynamic library loading to handle various module formats safely.
- **Defensive Data Checks:** Added explicit validation for payslip data and library instances before processing.
- **Guarded Property Access:** Implemented safety checks before accessing internal library state (like `lastAutoTable`) to prevent crashes during complex layouts.
- **Improved Monitoring:** Added detailed console error logging for PDF export failures to assist with rapid diagnosis in production.
- **Extended Fix:** Proactively applied these same robustness improvements to the **Compliance Reports** page to ensure system-wide reliability.

### UI Polish & Theme Visibility
- **Toast Notifications**: Resolved a visibility issue in light mode where error message text was hard-coded to white on a white background. It now dynamically adjusts to `var(--text-main)`, ensuring perfect readability and contrast across both light and dark themes.

✅ **Verification**: Readability confirmed in both modes via UI testing.
