...
- **Real-Time Database Synchronization**: KET updates are No longer isolated. Saving a KET document now automatically synchronizes critical data (Job Title, Basic Salary, Date Joined, CPF status, and Allowances) back to the primary **Employee Database** in a single transaction.
- **Input Stabilization**: Resolved a UI bug where input fields lost focus after a single character. The form architecture was refactored to ensure high performance and smooth typing during data entry.
- **Improved Data Integrity**: Implemented multi-layered fallback logic to ensure the **Employer/Entity Name** is always correctly displayed in both the UI and the generated PDF.
