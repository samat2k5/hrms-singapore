...
router.post('/', authMiddleware, async (req, res) => {
    try {
        const db = await getDb();
        const { employee_grade, leave_type_id, base_days, increment_per_year, max_days } = req.body;
        ...
        db.run(
            `INSERT INTO leave_policies (entity_id, employee_grade, leave_type_id, base_days, increment_per_year, max_days) 
             VALUES (?, ?, ?, ?, ?, ?) 
             ON CONFLICT(entity_id, employee_grade, leave_type_id) 
             DO UPDATE SET base_days=excluded.base_days, increment_per_year=excluded.increment_per_year, max_days=excluded.max_days`,
            [req.user.entityId, employee_grade, leave_type_id, base_days || 0, increment_per_year || 0, max_days || 0]
        );
        saveDb();
