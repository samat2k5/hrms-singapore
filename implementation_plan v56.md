...
- Update Detailed Timesheet UI table with "Basic hrs" and "PH hrs" columns.
- Update PDF export:
    - Replace default ezyHR logo with entity's `logo_url` in the header.
    - Append "Basic hrs" and "PH hrs" to the Detailed Timesheet table.
    - Add small ezyHR logo and "Powered by ezyHR" text to the footer.
...
- Update PDF export:
    - Add entity's `logo_url` to the header.
    - Add small ezyHR logo and "Powered by ezyHR" text to the footer.

#### [MODIFY] [Entities.jsx](file:///c:/Users/mathi/Desktop/AntiGravity%20Demos/HRMS%20Singapore/client/src/pages/Entities.jsx)
- Add field for "Logo URL" in the entity modal to allow users to specify their branding.

#### [MODIFY] [server/routes/entities.js](file:///c:/Users/mathi/Desktop/AntiGravity%20Demos/HRMS%20Singapore/server/routes/entities.js)
- Update `GET /` and `PUT /:id` to handle `logo_url`.

#### [NEW] `server/db/migrations/add_entity_logo.js`
- Add `logo_url` column to `entities` table.
