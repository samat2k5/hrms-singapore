...
### Entity Filtering
The Leave page now correctly filters both balances and requests based on the currently selected entity. Switching entities triggers an automatic re-fetch of data.

### PDF Report Improvements
Both Summary and Individual PDF reports now prominently display "Leave Balance as of: [Date]" above the data tables, as well as a timestamped "Report generated on" footer.

````carousel
```javascript
// Example of entity filtering in backend
const entityId = req.user.entityId;
const empResult = db.exec(`SELECT id FROM employees WHERE entity_id = ? AND status = 'Active'`, [entityId]);
```
<!-- slide -->
```javascript
// frontend reactivity
